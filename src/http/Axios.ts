...
import type { RequestOptions, Result, UploadFileParams, UploadFileCallBack } from '@/type/axios.d';
...
export interface UploadFileCallBackWithSuccess extends UploadFileCallBack {
  success?: (data: any) => void; // 添加success字段
  method?: string;
}
/**
...
export class VAxios {
  ...
  uploadFile<T = any>(config: AxiosRequestConfig, params: UploadFileParams, callback?: UploadFileCallBack) {
    ...
    if (params.data) {
      Object.keys(params.data).forEach((key) => {
        ...
        formData.append(key, (params.data as any)[key]);
      });
    }
  ...
  request<T = any>(config: UploadFileCallBackWithSuccess, options?: RequestOptions): Promise<T> {
   ...
   * @param formData
   */
  uploadMyFile<T = any>(url: string, formData: Recordable) {
    const glob = useGlobSetting();
